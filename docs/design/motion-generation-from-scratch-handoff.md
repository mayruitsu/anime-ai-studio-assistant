...
- **方式転換（2026-09-09）**：UIでの目視確認後もなお動きとカテゴリ名が一致しにくいというフィードバックがあり、追加学習では改善しないと判断。ユーザー提案により「現在の姿勢→次の姿勢を1ステップ提案し、人間が毎ステップ手直ししてキーフレームをつなげる」方式（`pose-suggestion-api`）に転換し、UI連携まで完了した。40ステップの完全自動連鎖はドリフト（誤差蓄積）で崩壊することが分かっているため、あえて1ステップずつ・人の確認を挟む設計にしている。詳細：`self-model-experiment/docs/tech/frame-transition-model.md`（self-model-experiment PR #71〜#76、本リポジトリ PR #84〜#86）
