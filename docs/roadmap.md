# ロードマップ

## 凡例
- ✅ 完了
- 🔄 進行中
- ⬜ 未着手

---

## フェーズ1：環境構築・ルール整備

| タスク | 状態 |
|---|---|
| README作成 | ✅ |
| PRテンプレート・差分チェックActions | ✅ |
| CLAUDE.md作成 | ✅ |
| Docker環境構成 | ✅ |
| docsディレクトリ構成 | ✅ |

## フェーズ2：Python学習・バックエンド基礎

> ※ UIの全体像を先に作るためフェーズ4を先行した。フェーズ4完了済みのため、ここから再開する。

| タスク | 状態 |
|---|---|
| Python環境構築（Docker内） | ✅ |
| Python基礎学習 | ✅ |
| FastAPI最小構成 | ✅ |
| MediaPipe導入・動作確認 | ✅ |

## フェーズ3：ポイント検出

| タスク | 状態 |
|---|---|
| 人型画像から関節33点を自動検出 | ✅ |
| 検出結果をAPIで返す | ✅ |

## フェーズ4：UIエディタ

| タスク | PR | 状態 |
|---|---|---|
| React環境構築（Vite + Docker） | #05 | ✅ |
| 画像アップロードUI | #06 | ✅ |
| Canvas上にモック関節ポイントを表示 | #07 | ✅ |
| ポイントをドラッグで動かせる | #08 | ✅ |
| フレーム追加・切り替え | #09 | ✅ |
| フロントからポーズ検出APIを呼び出し | #17 | ✅ |

## フェーズ5：動画出力

| タスク | 状態 |
|---|---|
| フレームをOpenCVで動画に変換 | ✅ |
| 動画ダウンロード機能 | ✅ |

## フェーズ6：キャラクターアニメーション高度化（MVP）※一旦保留

詳細設計: [docs/design/character-animation-concept.md](design/character-animation-concept.md) 参照。2D剛体パーツ切り抜き方式の技術検証として実施。ダイナミックな動きへの構造的な限界が判明し、[docs/design/3d-vrm-pivot.md](design/3d-vrm-pivot.md) の方針転換によりフェーズ8・9へ主軸を移す。実装自体は削除せず保持。

| タスク | 状態 |
|---|---|
| 人物切り抜き（セグメンテーション） | ✅ |
| パーツ分割の定義 | ✅ |
| パーツ画像の切り出し | ✅ |
| ポーズ→回転角の計算 | ✅ |
| フレーム合成（レンダリング） | ✅ |
| 動画出力の差し替え | ✅ |

## フェーズ7：キャラクターアニメーション高度化（発展）※一旦保留

詳細設計: [docs/design/character-animation-concept.md](design/character-animation-concept.md) の「後回しリスト」参照。フェーズ6同様、方針転換により一旦保留。

| タスク | 状態 |
|---|---|
| 複数写真でのモデル化・パーツの使い分け | 🔄 |
| イラスト（単色背景）向けの色ベース切り抜き対応 | ✅ |
| 写真間のズレ（光の当たり方・体の向き）の吸収 | ⬜ |
| 表情変化（口・目の状態差し替え） | ⬜ |
| 髪の動き（物理シミュレーション） | ⬜ |
| フレーム補間方式の作り込み（2コマ・3コマ打ちなど） | ⬜ |
| 背景との合成、フレームごとの背景設定 | ⬜ |
| 背景のパン・ズーム・回転演出（魚眼歪み） | ⬜ |
| 背景の一部変化・レイヤー分けパララックス | ⬜ |
| 可動域を外れたポーズを指定された場合の挙動 | ⬜ |

## フェーズ8：3Dモデルポージング＆アニメーション（トラックA）

詳細設計: [docs/design/3d-vrm-pivot.md](design/3d-vrm-pivot.md) 参照。既製のVRM（3Dアバター）モデルを使い、関節操作・アニメ的レンダリングでアニメーションを作成する。

| タスク | 状態 |
|---|---|
| Three.js + React環境構築 | ✅ |
| VRMモデルの読み込み・表示 | ✅ |
| 骨格取得・ポインターでの関節操作UI | ✅ |
| ポーズ変更に応じたレンダリング | ✅ |
| アニメ的レンダリング（トゥーンシェーディング） | ✅（MToonマテリアルによりVRM標準で対応済み） |
| フレーム間引き・動画出力 | ✅ |

## フェーズ9：AIによるモーション自動生成

テキストから動き（関節角度シーケンス）を生成する学習済みAIモデル（MDM: Motion Diffusion Model）を使い、手動キーフレーム操作に頼らずダイナミックな動きを作る。座標・角度を返すAIであり画像は生成しないため、既存方針と矛盾しない。

| タスク | 状態 |
|---|---|
| MDMの動作検証（GPU環境構築、テキストからのモーション生成） | ✅ |
| SMPL回転パラメータの取得（joints2smpl/SMPLify） | ✅ |
| SMPL関節→VRM humanoidボーンへのリターゲティング | ✅ |
| アプリへの組み込み（生成結果の再生・動画書き出し） | ✅ |
| バックエンドAPI化（ブラウザから直接生成をリクエストできるようにする） | ✅ |

MDMの実行環境（PyTorch・GPU前提）はこのリポジトリの外（別ディレクトリ）で構築。セットアップ方法は [docs/tech/motion-api.md](tech/motion-api.md) 参照。1回の生成に2〜3分程度かかる（SMPLifyの最適化処理がボトルネック）。

### 自前モーション生成モデルへの移行（2026-09-09〜）

MDMの学習データ（HumanML3D）が商用利用不可のライセンスと判明したため、CMU Graphics Lab Motion Capture Databaseで学習した自前モデルへの移行に着手（詳細：[docs/design/motion-generation-from-scratch-handoff.md](design/motion-generation-from-scratch-handoff.md)）。「1文→全フレーム一括生成」の拡散モデル方式を経て、「現在の姿勢→次の姿勢を1ステップ提案し、人間が毎ステップ手直しする」方式（`pose-suggestion-api`）に転換し、UI連携まで完了。将来的にmotion-apiを置き換える想定。

## フェーズ10：写真から3Dモデル生成の研究（トラックB）

時間のかかる探索的タスク。他フェーズの進行をブロックしない。別リポジトリ [self-model-experiment](https://github.com/mayruitsu/self-model-experiment) で進行中（ローカルパス: `self-model-experiment`）。

学習済みAIモデル・新規AIモデル学習のどちらにも依存せず、Visual Hull（シルエット交差）と微分可能レンダリング（陰影最適化）による物理ベースの3D復元を行う方針に転換済み。最終的な入力は実写ではなく4方向（正面・右90°・背面・左90°）のイラスト。

| タスク | 状態 |
|---|---|
| 環境構築（nvdiffrast導入・GPU動作確認、撮影プロトコル決定） | ✅ |
| テスト用写真の撮影・シルエット抽出の動作確認 | ⬜ |
| Visual Hullによる粗形状復元 | ⬜ |
| 陰影による形状の詳細化（単一視点・複数視点とも不安定と判明、追加の正則化設計を検討中） | 🔄 |
| 骨格設定・VRM変換 | ⬜ |
| 既存アプリ（本リポジトリ）への統合テスト | ⬜ |

詳細は`self-model-experiment`リポジトリの `docs/roadmap.md`・`docs/progress.md` 参照。

## フェーズ11：GCPデプロイ

| タスク | 状態 |
|---|---|
| Cloud Runへのデプロイ | ⬜ |
| Cloud Storageへの画像・動画保存 | ⬜ |
